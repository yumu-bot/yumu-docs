...
在 MRA v3.4 中，根据贡献 (RWS，Rounds Win Share，对局胜利分享)，玩家被更加细致地分为 33 个扩展类型，力求给玩家最精准的评价。

玩家的 RWS 越高，则他在自己队伍中做出的贡献（有效输出）越多。反之则越少。
...
输出玩家在某比赛房间内的特定一个或所有成绩。

**使用方法**

!ymmatchrecent / mr [**比赛编号**] (**玩家名**) (`#`**位置**)
...
- **<HopeIcon icon="address-card"/> 玩家名**：需要查询的玩家名称。留空默认查询自己。
  ...
  - 如果玩家名符合比赛编号的定义（7-9 位数），则会自动按比赛编号查询自己的最近成绩。
- **<HopeIcon icon="map-marker"/> 位置**：如果输入一个值，则会返回这一个位置的成绩。
  - 不输入默认返回 50 个成绩。
