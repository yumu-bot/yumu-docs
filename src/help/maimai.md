...
!ymmaimaiscore / ms (**玩家名**) (**种类**) (**歌曲名称 / 歌曲编号**) (**查询条件**) (**页数**)

- **<HopeIcon icon="address-card"/> 玩家名**：同上。
    - 支持使用 <HopeIcon icon="fa-brands fa-qq"/> QQ 查询，输入格式为：qq=114514。
- **<HopeIcon icon="rainbow"/> 种类**：允许按歌曲种类查询。
  - 支持输入标准（SD、standard）、豪华（DX、deluxe）
  - 留空默认按歌曲支持的种类查询。如果有多个种类，则按查询玩家曾经留下的，最高 DX 评分成绩所属的歌曲种类查询。
- **<HopeIcon icon="address-card"/> 歌曲名称**：二选一填，需要查询的歌曲名。
  - 支持输入简中社区常用的外号（俗称）。	
...
- **<HopeIcon icon="filter"/> 查询条件**：输入格式为：==条件 + 运算符 + 查询内容==，比如 mapper=fia。
  - 可输入的条件种类有：
    - 谱师 `charter`/`chart`/`c`
      - 可以输入常见的谱师外号，比如 ニャイン 可以通过输入 二爷 查询。
      - 如果不清楚外号，也可以输入全名。
    - 歌曲编号 `id`/`i`
    - 难度分类或难度定数 `difficulty`/`d`
    - 框体 `cabinet`/`n`
      - 可以输入的参数：SD、DX
    - 版本 `version`/`v`
      - 等同于上述的版本号。
    ...
    - 别名 `alias`/`l`
      - 等同于上述的外号。
    - 种类 `type`/`t`
      - 可以输入的参数：流行、动漫、东方、v家、舞萌、音击、中二、宴会场。
      - 或是 pop、anime、touhou、vocaloid、niconico、maimai、ongeki、chunithm、utage。
      - 或是 p、a、t、v、n、m、o、c、u。
    - 曲速 `bpm`
    - 达成率 `achievement`/`ach`/`acc`
    - 物件数量 `tap`, `hold`, `slide`, `touch`, `break`
    - DX 分数
      - 非常完美 (CP) 计 3 分，完美 (PF) 计 2 分，良好 (GR) 计 1 分。
    - DX 星
      - 可以输入的范围：0-5。
    - 评分 `rating`
      - 即单曲 ra，比如：定数 13.7 的谱面，对应的完美评分是 308。
    - 评价 `rank`
      - 可以输入的评价：SSS+ ~ D。
      - 如果使用除了相等、完全相等、不相等之外的运算符，则会按以上所示，从左往右依次递减排序。
      - 比如，输入 rank>ss+，会返回评级为 SSS、SSS+（均在 SS+ 左侧，比 SS+ 更好）的成绩。
    - 连击状态 `combo`
      - 可以输入的参数：fc, fc+, ap, ap+
    - 同步状态 `sync`
      - 可以输入的参数：sync, fs, fs+, fdx(fsd), fdx+
